...
const sb = {
  ...
  async upsert(table, row) {
    ...
    if (patchRes.ok) {
      // 檢查是否有更新到（204表示成功但可能沒找到資料）
      const postRes = await fetch(`${SUPABASE_URL}/rest/v1/${table}?id=eq.${row.id}&select=id`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
      });
      const existing = await postRes.json();
